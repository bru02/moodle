...
import { useOptionalDismissibleItemsContext } from "./DismissibleItemsContext";
...
export function HiddenItemActionsSection<T>({ item }: { item: T }) {
  ...
  const canDismiss = dismissibleCtx?.isItemDismissible(item) ?? false;

  return (
    <ActionPanel.Section>
      {canDismiss && (
        <Action
          title="Dismiss from New"
          icon={Icon.XMarkCircle}
          onAction={() => dismissibleCtx?.dismissItem(item)}
