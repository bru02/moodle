...
export function useHiddenItemsState(namespace: string): HiddenItemsStateResult {
  const storedItems = useHiddenItemsStore(
    (state) => state.itemsByNamespace[namespace] ?? EMPTY_HIDDEN_ITEMS,
  );
  ...
  const [showingHidden, toggleShowingHidden] = useReducer(
    (state) => !state,
    false,
  );
  ...
  const isHiddenKey = useCallback(
    (itemKey: HiddenItemKey) => hiddenItemSet.has(itemKey),
    [hiddenItemSet],
  );
  const isPinnedKey = useCallback(
    (itemKey: HiddenItemKey) => pinnedItemSet.has(itemKey),
    [pinnedItemSet],
  );
  ...
  const setHiddenKeys = useCallback(
    (itemKeys: readonly HiddenItemKey[], value: boolean) =>
      setStoredItems(namespace, itemKeys, false, value),
    [namespace, setStoredItems],
  );
  const setPinnedKeys = useCallback(
    (itemKeys: readonly HiddenItemKey[], value: boolean) =>
      setStoredItems(namespace, itemKeys, true, value),
    [namespace, setStoredItems],
  );
...
export function useHiddenItems<T>({
  ...
  namespace,
  getItemKey,
}: UseHiddenItemsOptions<T>): UseHiddenItemsResult<T> {
  const {
    hiddenItems,
    pinnedItems,
    hiddenItemSet,
  ...
  const isItemHidden = useCallback(
    (item: T) => hiddenItemSet.has(getItemKey(item)),
    [getItemKey, hiddenItemSet],
  );
  const isItemPinned = useCallback(
    (item: T) => pinnedItemSet.has(getItemKey(item)),
    [getItemKey, pinnedItemSet],
  );

  const toggleItem = useCallback(
    (item: T) => toggleHiddenKey(getItemKey(item)),
    [getItemKey, toggleHiddenKey],
  );
  const togglePinnedItem = useCallback(
    (item: T) => togglePinnedKey(getItemKey(item)),
    [getItemKey, togglePinnedKey],
  );

  const { visibleData, pinnedVisibleData, regularVisibleData, hasVisibleData } =
    useMemo(() => {
      const nextVisible: T[] = [];
      const nextPinned: T[] = [];
      const nextRegular: T[] = [];
