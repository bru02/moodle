...
const wsClient = createMoodleWSClient({
  ...
  resolveRequestParams: ({ params, session }) =>
    "userid" in params && params.userid === 0
...
type WSBatchQueryKey<K extends keyof WSParamsMap> = readonly [
  K,
  "batch",
  readonly WSParamsMap[K][],
];

type WSQueryOptions<
  K extends keyof WSParamsMap,
  TData = WSResponseMap[K],
> = Omit<
  UseQueryOptions<WSResponseMap[K], Error, TData, WSQueryKey<K>>,
  "queryKey" | "queryFn"
>;

type WSSuspenseQueryOptions<
  K extends keyof WSParamsMap,
  TData = WSResponseMap[K],
> = Omit<
...
function getQueryOptions<K extends keyof WSParamsMap>(
  key: K,
  params: WSParamsMap[K],
) {
  return wsClient.getQueryOptions<WSResponseMap[K]>(
    key,
...
function getBatchQueryOptions<K extends keyof WSParamsMap>(
  key: K,
  paramsList: readonly WSParamsMap[K][],
) {
  return wsClient.getBatchQueryOptions<WSResponseMap[K]>(
    key,
    paramsList as readonly RequestParams[],
  ) as unknown as {
...
export function useWSQuery<
  K extends keyof WSParamsMap,
  TData = WSResponseMap[K],
>(key: K, params: WSParamsMap[K], options?: WSQueryOptions<K, TData>) {
...
export function useWSBatchQuery<
  K extends keyof WSParamsMap,
  TData = WSResponseMap[K][],
>(
  key: K,
  ...
  return useQuery<WSResponseMap[K][], Error, TData, WSBatchQueryKey<K>>(
    { ...baseOptions, ...options },
    queryClient,
  );
}

export function useSuspenseWSQuery<
  K extends keyof WSParamsMap,
  TData = WSResponseMap[K],
>(key: K, params: WSParamsMap[K], options?: WSSuspenseQueryOptions<K, TData>) {
  const baseOptions = getQueryOptions(key, params);
